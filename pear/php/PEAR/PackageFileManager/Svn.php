 ...
 * @version   CVS: $Id: Svn.php,v 1.18 2008/12/29 04:57:38 dufuz Exp $
...
class PEAR_PackageFileManager_Svn extends PEAR_PackageFileManager_File
{
    ...
    function _readSVNEntries($entries)
    {
        ...
        foreach ($entries as $svnentry) {
            $directory = @dirname(@dirname($svnentry));
            if (!$directory) {
                continue;
            }

            $d = $this->_getSVNEntries($svnentry);
            if (!is_array($d)) {
                continue;
            }
            ...
            foreach ($d as $entry) {
                ...
                if ($ignore) {
                    if ($this->_checkIgnore($entry, $file, 1)) {
                        continue;
                    }
                }

                if ($include) {
                    if ($this->_checkIgnore($entry, $file, 0)) {
                        continue;
                    }
                }
