SYMLINK = horde/framework/bin/install_framework
...
MODULES = horde horde-fw3 kolab-cvs topgit
...
$(TEST_HEAD_PKGS:%=test2-HEAD-%): lib
	...
	if [ -n "$$ALL_TESTS" ]; then \
	  ...
	  for TEST in $$ALL_TESTS; do \
	    cd $$TEST && phpunit -d include_path=".:$$CWD/lib:$$CWD/pear/php:/usr/share/php5:/usr/share/php" -d log_errors=1 -d error_log="$$CWD/logs/php-errors.log" AllTests.php 2>&1 | tee -a $$CWD/log/$@-HEAD-phpunit.log | grep "^OK" > /dev/null || PHPUNIT="FAIL"; \
	    cd $$CWD; \
	  done; \
	  if [ -n "$$PHPUNIT" ]; then \
	    echo; \
...
$(TEST_CVS_PKGS:%=test-CVS-%): lib
	@PHP_FILES=`find horde/framework/$(@:test-CVS-%=%)/ -name '*.php'`; \
	if [ -n "$$PHP_FILES" ]; then \
	...
	@SIMPLE_TESTS=`find horde/framework/$(@:test-CVS-%=%)/ -name '*.phpt' | xargs -L 1 -r dirname | sort | uniq`; \
	...
	@ALL_TESTS=`find horde/framework/$(@:test-CVS-%=%)/ -name AllTests.php | xargs -L 1 -r dirname | sort | uniq`; \
	if [ -n "$$ALL_TESTS" ]; then \
	  CWD=`pwd`; \
	  rm -f log/$@-CVS-phpunit.log; \
	  for TEST in $$ALL_TESTS; do \
	    cd $$TEST && phpunit -d include_path=".:$$CWD/lib:$$CWD/pear/php:/usr/share/php5:/usr/share/php" -d log_errors=1 -d error_log="$$CWD/logs/php-errors.log" AllTests.php  2>&1 | tee -a $$CWD/log/$@-CVS-phpunit.log | grep "^OK" > /dev/null || PHPUNIT="FAIL"; \
	    cd $$CWD; \
	  done; \
	  if [ -n "$$PHPUNIT" ]; then \
	    echo; \
...
revcmp-horde:
	...
	do                       \
	  $(REVBIN) horde-fw3/framework/$$MODULE/ horde/framework/$$MODULE/; \
	done
...
test-framework:
	php -c php.ini -f horde/framework/bin/test_framework
